...
def plot_confusion_matrix(conf_matrix, labels,fname):
    ...
    plt.show()
    plt.savefig("results/folds_stacked/fold5_"+fname+"_Conf_mat.png")
    plt.close()
...
if __name__ == '__main__':
    ...
    test_dataset = tf.data.Dataset.from_generator(
        lambda: process_each_file_StackedModel(test_directory_path, config),
    ...
    mod_name = 'best_model_withTemp_removedNS_highpass01_order5_NoPeakRemoval_ppghighpass02low5_RobScaleAll_BinFocalLossG4a02_lrdecayCos12ka01_win60_step30_batch32_FilterSegment30_lr0.0001'
    
    
    model = tf.keras.models.load_model('models/folds_stacked/'+mod_name+'.h5')
    ...
    plot_roc_curve(fpr, tpr, roc_auc, filename='results/folds_stacked/fold5_'+fname+'_roc_curve.png')
    ...
    with open(f'results/folds_stacked/fold5_{fname}_classification_report.txt', 'w') as f:
        # Write label counts
